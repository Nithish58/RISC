...
public class ReinforceGameController implements Controller{
    ...
    public void tradeNone(Player player, String[] commands){
        if(!commands[1].equalsIgnoreCase("-none")){
            throw new ReinforceParsingException(commands[1] + " " + commands[2] + " is not valid");
        }
    ...
    public void tradeInCards(Player player, int cardOne, int cardTwo, int cardThree) {
        ...
        cardExchangeView.displayMessage("the number of armies received from cards exchange is " + player.getTradeInTimes() * 5);
        //cardExchangeView.displayMessage("the total number of reinforced armies received is: " + reinforcedArmies);
